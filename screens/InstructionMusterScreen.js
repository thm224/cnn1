...
import {getDay} from "../constants/Api"
import { Header } from 'react-native-elements';

export default class InstructionMusterScreen extends Component {
    _isMounted = false;
  ...
  get_day = async(Musterclass) => {
    // alert(Musterclass)
    console.log (Musterclass)
    response = await getDay({"classID":Musterclass});
    this.setState({days:response.days})
  ...
  render() {
    return (
      <View style={styles.screen}>
        ...
        centerComponent={<Text style={[theme.fonts.bold20, { color: "#fff" }]}>Chọn lớp</Text>}
        // rightComponent={{ icon: 'home', color: '#fff' }}
      />
        <View style={[styles.container,]}>
          {/* <Text style={{fontSize: 20}}>Chọn Lớp</Text> */}
          <Picker
        ...
        <View style={styles.footer}>
          <TouchableOpacity style={styles.nextbutton}  onPress = {() => {NavigationUtil.navigate("MusterCaptureScreen");
          console.log(this.state.Musterclass);
          global.musterClass = this.state.Musterclass}}>
